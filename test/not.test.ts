  ...
  describe('not.any()', () => {
    describe('basic behaviour', () => {
      it('without conditions', () => {
        const result = tendency(not.any('a', 'b', 'c'))
        expect(result).to.equal('a b c')
      })

      it('without conditions multiple calls', () => {
        const result = tendency(not.any('a'), not.any('b'), not.any('c'))
        expect(result).to.equal('a b c')
      })

      it('only valid conditions', () => {
        const result = tendency(true, not.any('a', 'b', 'c'))
        expect(result).to.equal('a b c')
      })

      it('only valid conditions multiple calls', () => {
        const result = tendency(true, not.any('a'), not.any('b'), not.any('c'))
        expect(result).to.equal('a b c')
      })

      it('only invalid conditions', () => {
        const result = tendency(false, not.any('a', 'b', 'c'))
        expect(result).to.equal('a b c')
      })

      it('only invalid conditions multiple calls', () => {
        const result = tendency(false, not.any('a'), not.any('b'), not.any('c'))
        expect(result).to.equal('a b c')
      })

      it('mixed conditions', () => {
        const result = tendency(true, false, not.any('a', 'b', 'c'))
        expect(result).to.equal('a b c')
      })

      it('mixed conditions multiple calls', () => {
        const result = tendency(true, false, not.any('a'), not.any('b'), not.any('c'))
        expect(result).to.equal('a b c')
      })
    })
    ...
    describe('nested behaviour', () => {
      ...
      it('complex without conditions', () => {
        const result = tendency(not.any('a', 'b', not.any('c', 'd', not.any('e', 'f'), 'g'), 'h'))
        expect(result).to.equal('a b c d e f g h')
      })

      it('valid conditions', () => {
        const result = tendency(true, not.any(true, 'a', not.any(true, 'b', not.any(true, 'c'))))
        expect(result).to.equal('a b c')
      })

      it('complex valid conditions', () => {
        const result = tendency(true, not.any(true, 'a', 'b', not.any(true, 'c', 'd', not.any(true, 'e', 'f'), 'g'), 'h'))
        expect(result).to.equal('a b c d e f g h')
      })

      it('invalid conditions', () => {
        const result = tendency(false, not.any('a', [false, not.any('b', [false, not.any('c')])]))
        expect(result).to.equal('a b c')
      })

      it('mixed conditions', () => {
        const result = tendency(true, not.any('a', [false, not.any('b', [true, not.any('c')])]))
        expect(result).to.equal('a b c')
      })
    })
  })
